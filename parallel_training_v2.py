...
def plot_results(acc, loss, iterations, legends, scheme):
    ...
    image_name = home_dir + 'Outputs/EuroSatSNN_Comparison_demo_' + scheme + '_para_loss_v2.pdf'
    ...
    image_name = home_dir + 'Outputs/EuroSatSNN_Comparison_demo_' + scheme + '_para_accuracy_v2.pdf'
    fig.savefig(image_name, format='pdf', dpi=1200)
    plt.show()


if __name__ == '__main__':
    from config import args3
    ...
    print('training epochs: ' + str(args3.num_epoch) + ' plane-alpha: ' + str(args3.plane_alpha))
    ...
    # indices_per_plane = Dirichlet_non_iid_distribution(targets, args3.plane_alpha, num_planes,
    ...
    #     num_satellites) + '_alpha_' + str(args3.plane_alpha) + '_auxi_' + str(3) + '_TrainSetPartition.pkl', 'wb') as f:
    #     pickle.dump(local_dataset_indices, f)

    with open(home_dir + 'Outputs/EuroSat_SNN_planes_' + str(num_planes) + '_satellites_' + str(
        num_satellites) + '_alpha_' + str(args3.plane_alpha) + '_auxi_' + str(3) + '_TrainSetPartition.pkl', 'rb') as f:
        local_dataset_indices = pickle.load(f)

    constellation = ConstellationLearning(num_planes, satellites_by_plane, train_set, test_set, local_dataset_indices, args3)
    # constellation.dataset_partition()

    acc = numpy.zeros((3, repeat, args3.num_epoch))
    loss = numpy.zeros((3, repeat, args3.num_epoch))
    training_loss = numpy.zeros((3, repeat, args3.num_epoch))
    test_loss = numpy.zeros((3, repeat, args3.num_epoch))
    saved_acc = numpy.zeros((3, args3.num_epoch))
    saved_loss = numpy.zeros((3, args3.num_epoch))
    saved_training_loss = numpy.zeros((3, args3.num_epoch))
    saved_test_loss = numpy.zeros((3, args3.num_epoch))
    ...
    for r in range(repeat):
        ...
        #
        # for epoch in range(args3.num_epoch):
        ...
        #
        # out_file_name = home_dir + 'Outputs/EuroSat_SNN_InterPlane_Comparison_T_' + str(args3.T) + '_alpha_' + str(
        #     args3.plane_alpha) + '_num_planes_' + str(num_planes) + '_lr_' + str(args3.lr) + '_repeat_' + str(r) + '_RelaySum_para_results_v2.npz'
        ...
        # out_file_name = home_dir + 'Outputs/EuroSat_SNN_InterPlane_Comparison_T_' + str(args3.T) + '_alpha_' + str(
        #     args3.plane_alpha) + '_num_planes_' + str(num_planes) + '_lr_' + str(args3.lr) + '_repeat_' + str(r) + '_Gossip_para_results_v2.npz'
        # numpy.savez(out_file_name, acc=saved_acc, loss=saved_loss, trainloss=saved_training_loss,
        #             testloss=saved_test_loss)
        ...
        out_file_name = home_dir + 'Outputs/EuroSat_SNN_InterPlane_Comparison_T_' + str(args3.T) + '_alpha_' + str(
            args3.plane_alpha) + '_num_planes_' + str(num_planes) + '_lr_' + str(args3.lr) + '_repeat_' + str(r) + '_AllReduce_para_results_v2.npz'
        numpy.savez(out_file_name, acc=saved_acc, loss=saved_loss, trainloss=saved_training_loss,
                    testloss=saved_test_loss)
    plot_results(acc, loss, args3.num_epoch, legends, scheme)
